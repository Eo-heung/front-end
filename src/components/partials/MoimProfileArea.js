  ...
  const fetchMoimData = async () => {
    try {
      ...
      const response1 = await axios.get(
        `${SPRING_API_URL}/moimReg/view-moim-profile/${moimId}`,
        {
          headers: {
      ...
      console.log(
        "data.moimProfileBase64",
        response1.data.item.msg.moimProfileBase64
      );
    } catch (err) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const response = await axios.post(
        `${SPRING_API_URL}/moimReg/modify-moim-profile/${moimId}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("ACCESS_TOKEN")}`,
      ...
      setSelectedImagePreview(
        blobToFile(response.data.item.moimProfile, "aaa")
      );
  ...
  return (
    <div className="sb-sidenav-profile">
      ...
      <div className="sidenav-profile-appoint">
        <span style={{ fontSize: "1.2rem", marginLeft: "90px" }}>
          {moimData.moimTitle}
        </span>
        <br />
        <div
      ...
      <div className="sidenav-profile-appointList">
        <div className="sidenav-profile-appointListItem">
          ...
          <InfoZone>
            <MyInfoLink to={`${moimId}/moim-board/my-moim-info`}>
              내 글 관리
            </MyInfoLink>
          </InfoZone>
        </div>
