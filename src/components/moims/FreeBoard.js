...
const FreeBoard = () => {
  ...
  const isLoginUserTheWriter =
    boardDetail && boardDetail.userId === cookie.userId;
  ...
  useEffect(() => {
    const fetchBoardDetail = async () => {
      try {
        const response = await axios.post(
          `${SPRING_API_URL}/board/${moimId}/view-board/${boardId}`,
          {},
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("ACCESS_TOKEN")}`,
  ...
  const handleEditClick = () => {
    navi(`/${moimId}/create-board/${boardId}`, {
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      const response = await axios.post(
        `${SPRING_API_URL}/board/${moimId}/delete-board/${boardId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("ACCESS_TOKEN")}`,
  ...
  return (
    <BoardContainer>
      ...
      <BoardInfoRow>
        ...
        <BoardInfo>
          {boardDetail && boardDetail.boardRegdate.slice(0, 10)}
        </BoardInfo>
      </BoardInfoRow>
