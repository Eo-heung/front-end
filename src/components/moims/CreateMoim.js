      ...
      const createMoimAxios = async () => {
        ...
        try {
          const response = await axios.post(
            `${SPRING_API_URL}/moim/create-moim`,
            userData,
            {
              headers: {
      ...
      >
        <StyledBox>
          ...
          <Box border={0} my={0} display="flex" alignItems="center">
            <h5 fontWeight="bold" style={{ width: "110px" }}>
              모임장
            </h5>
          ...
          <StyledTextField
            name="maxMoimUser"
            placeholder="최대 50명까지 모집할 수 있어요."
            onChange={handleInputChange}
            variant="outlined"
          />
          ...
          <StyledTextField
            name="moimContent"
            placeholder="주제 중심으로 모임을 소개해주세요. 모임 설정에서 언제든지 바꿀 수 있어요."
            onChange={handleInputChange}
            variant="outlined"
            multiline
            rows={4}
          ...
          {moimPic ? (
            ...
            <ImageAttaZone>
              <Button
                onClick={triggerFileInput}
                style={{ color: grey[600], padding: "11% 42.1%" }}
              >
